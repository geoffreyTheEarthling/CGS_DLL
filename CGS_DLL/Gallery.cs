...
namespace CGS_DLL
{
    public class Gallery
    {
        ...
        static string txtPath = @".\textFiles\SaveCuratorsInfo.txt";
        static string artistsfilePath = @".\textFiles\SaveArtistsInfo.txt";
        static string artpFilePath = @".\textFiles\SaveArtpiecesInfo.txt";
        ...
        public string addArtist(string artistID, string artistFName, string artistLName)
        {
            string message = null;
            ...
            else
            {
                ...
                {
                    if (artistFName.Length + artistLName.Length > 40)
                    {
                        message = "Error! Firstname and Lastname must be 40 characters maximum.";
                    }
                    else
                    {
                        Artist artist = new Artist(artistID, artistFName, artistLName);
                        myArtists.add(artist);
                        message = "Succces! Artist has been added to the list.";
                    }
                }
            }
        ...
        public string addArtPiece(string artpieceId, string curatorId, string artistID,
            string title, int year, double value)
        {
            string message = null;

            if (artpieceId.Length != 5)
            {
                message = "Error! The piece ID should be 5 characters";
            }
            else
            {
                if(pieceIdVerifier(artpieceId) == true)
                {
                    message = "Error! This ID already exists";
                }
                else
                {
                    if(curatorIdVerifier(curatorId) == false)
                    {
                        message = "Error! Curator ID is incorrect";
                    }
                    else
                    {
                        if(artistIdVerifier(artistID) == false)
                        {
                        ...
                        else
                        {
                            ...
                            {
                                message = "Error! Year should be 4 digits";
                            }
                            else
                            {
                                Artpiece artpiece = new Artpiece(artpieceId, curatorId, artistID, 
                                    title, year, value);
                                myArtpieces.add(artpiece);
                                message = "Succces! Art Piece has been added to the list.";
                            }
                        }
                    }
                }
        ...
        public string sellPiece(string pieceID, double estimate)
        {
            string message = null;
            if(pieceIdVerifier(pieceID) == false)
            {
                message = "Error! Artpiece ID does not exist";
            }
            else
            {
                if(statusVerifier(pieceID) == true)
                {
                    message = "Error! This artpiece is already tagged as Sold";
                }
                else
                {
                    if(estimate < getValueOfArtpiece(pieceID))
                    {
                        message = "Error! The entered price (estimate) is below the value";
                    }
                    else
                    {   
                        changeStatusToSold(pieceID);
                        string curatorID = getCuratorIdFromArtpieceID(pieceID);
                        setCommission(curatorID, getValueOfArtpiece(pieceID), estimate);
                        setEstimate(pieceID, estimate);
                        message = "Success! Artpiece " + pieceID + " is now sold";
                    }
                }
            }
        ...
        public string saveCuratorsInfo()
        {
            string textToWrite = null;
            for(int i = 0; i < myCurators.Count; i++)
            {
                textToWrite += myCurators[i].toString() + "\n";    
            }
            using (StreamWriter sw = new StreamWriter(txtPath, true ))
            {
                sw.WriteLine(textToWrite);
            }
            return "Curators successfully added to file";
        }

        public string saveArtistsInfo()
        {
            string textToWrite = null;           
            try
            {
                ...
                {
                    textToWrite += myArtists[i].toString() + "\n";
                }
                using (StreamWriter sw = new StreamWriter(artistsfilePath, true))
                {
                    sw.WriteLine(textToWrite);
                }
            }
            ...
            return "Artist successfully added to file";
        }

        public string saveArtPiecesInfo()
        {
            string textToWrite = null;
            try
            {
                ...
                {
                    textToWrite += myArtpieces[i].toString() + "\n";
                }
                using (StreamWriter sw = new StreamWriter(artpFilePath, true))
                {
                    sw.WriteLine(textToWrite);
                }
            }
            ...
            return "ArtPiece successfully updated";
        }

        //public void deleteArtpieceOnDisplay(string artpID)
        //{
        //    foreach(Artpiece artp in myArtpieces)
        //    {
        //        if(artp.ArtpieceId == artpID)
        //        {
        //            if(artp.Status == 'D')
        //            {
        //                myArtpieces.RemoveAt();
        //            }
        //        }
        //    }
        //}
